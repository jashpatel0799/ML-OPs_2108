...
parser.add_argument('--clf_model', type=str, required=True)
# parser.add_argument('--tree', type=str, required=True)
parser.add_argument('--random_state', type=int, required=False)
args = parser.parse_args()

X_train, y_train, X_dev, y_dev, X_test, y_test = train_dev_test_split(
    data, label, train_frac, dev_frac, 1 - (train_frac + dev_frac), args.random_state
)
...
if args.clf_model == "svm":
    ...
    file_1.write(f"svm f1_macro:{str(f1_score(y_test, predicted,  labels=None, pos_label=1, average='macro', sample_weight=None, zero_division='warn'))}")
    file_1.close()



elif args.clf_model == "tree":
    ...
    file_1.write(f"tree f1_macro: {str(f1_score(y_test, tree_pre,  labels=None, pos_label=1, average='macro', sample_weight=None, zero_division='warn'))}")
    file_1.close()
